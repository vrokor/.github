<div align="center">

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/vrokor/.github/main/brand/vrokor-logo-dark.png">
  <img src="https://raw.githubusercontent.com/vrokor/.github/main/brand/vrokor-logo-light.svg" alt="VROKOR" width="420">
</picture>

<br/><br/>

**Enterprise AI workflow orchestration.**

Secure, human-in-the-loop agent operations on Google Cloud —
per-tenant envelope encryption, automatic crypto-shredding, and an
operations console built for oversight.

</div>
